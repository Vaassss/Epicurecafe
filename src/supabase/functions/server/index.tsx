...
// Security: CORS - Restrict to your domain in production
...
const MASTER_ADMIN_MOBILE = Deno.env.get("MASTER_ADMIN_MOBILE") || "9999999999";
const MANUAL_ENTRY_CODE = Deno.env.get("MANUAL_ENTRY_CODE") || "CAFE2024";
const OTP_MAX_ATTEMPTS = 5; // Maximum OTP verification attempts
const OTP_RATE_LIMIT = 3; // Maximum OTP requests per phone per hour
const ADMIN_RATE_LIMIT = 100; // Maximum admin API calls per hour
...
interface OTPData {
  ...
  otp: string;
  expiresAt: number;
  attempts: number; // New: track OTP attempts
}
...
app.post("/make-server-6a458d4b/send-otp", async (c) => {
  try {
    ...
    const otpRateKey = `otp_rate:${mobile}`;
    const otpRateData = await kv.get<number>(otpRateKey);
    const currentHour = Math.floor(Date.now() / 3600000);
    if (otpRateData && otpRateData > currentHour) {
...
app.post("/make-server-6a458d4b/verify-otp", async (c) => {
  try {
    ...
    if (storedOTPData.otp !== otp) {
      ...
      if (storedOTPData.attempts >= OTP_MAX_ATTEMPTS) {
        await kv.del(`otp:${mobile}`);
        return c.json({ error: "Too many failed attempts. Please request a new OTP." }, 403);
      }
      await kv.set(`otp:${mobile}`, storedOTPData);
...
app.post("/make-server-6a458d4b/admin/add-admin", async (c) => {
  try {
    ...
    // Add to admin list
    await kv.set(`admin:${mobile}`, { mobile, name, addedAt: new Date().toISOString(), addedBy: adminMobile });
