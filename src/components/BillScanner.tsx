...
export function BillScanner({ userId, onClose, onSuccess }: BillScannerProps) {
  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      // Extract invoice number from bill
      const invoice = extractInvoiceNumber(text);
      ...
      if (foundItems.length === 0) {
        ...
        const invoiceMsg = invoice ? ` (Invoice #${invoice})` : '';
        toast.success(`Found ${foundItems.length} items in the bill!${invoiceMsg}`);
      }
    } catch (error) {
  ...
  const extractInvoiceNumber = (text: string): string => {
    // Look for patterns like "Invoice No: 641" or "Invoice No : 638"
    const patterns = [
  ...
  const extractMenuItems = (text: string): string[] => {
    ...
    // Check each menu item name and aliases
    menuItems.forEach(item => {
      ...
      // Check aliases if available and not already matched
      ...
      // Check partial matches (at least 70% of the word) if still not matched
      if (!matched) {
    ...
    if (response.error) {
      if ((response as any).isDuplicate) {
        const msg = invoiceNumber 
          ? `Invoice #${invoiceNumber} has already been scanned!`
          : 'This bill has already been scanned!';
        toast.error(msg);
      } else {
